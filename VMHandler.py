...
class VMConstructor:
    def createVM(VM: VM,networks,neutron: NeutronClient,nova: NovaClient):
        ...
        nova.create_instance_with_multiple_networks(nombre=VM.name,flavor_id = VM.flavorID, imagen_id = VM.imageID, keypair_id = VM.keyPairID, security_group_id = VM.securitygroupID, networks = networks)
